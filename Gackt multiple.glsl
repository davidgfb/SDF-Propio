/*
    ...
    Inspired by:       https://iquilezles.org/articles/genetic
...
vec4 plot(vec2 uv, vec4 colA, /*G g*/vec4 colB, vec3 b)
{
    /*vec4 colB = g.col_B; 
    vec3 b = g.b;*/

    vec2 p = vec2(mod(b.x*W*H,W)/W, floor(b.x*H)/H),
        d = vec2(mod(b.y*R*R,R)/R, floor(b.y*R)/R);
    
    p += (hash22(p)-.5)*2. / exp(iTime);
    
    float dos_Pi = 6.28, 
          s = sin(b.z*dos_Pi),c = cos(b.z*dos_Pi),r = max(b.x,b.y),
          f = colB.a * max(1.-length(mat2(c,s,-s,c)*(uv-p)/d*r)/r,0.);
...
vec4 plot(vec2 uv, vec4 colA, G g) {    
    return plot(uv, colA, g.col_B, g.b);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec4[] vs_4 = vec4[](vec4(0.381,0.288,0.160,0.571),
                         vec4(0.560,0.425,0.250,0.175));
    vec3[] vs_3 = vec3[](vec3(0.858816,0.367491,0.556801),
                         vec3(0.575926,0.211968,0.274033));
    
    G[] gs = G[] (G(vs_4[0], vs_3[0]),
                  G(vs_4[1], vs_3[1]));
	   
    for (int i = 0; i<gs.length(); i++) col = plot(uv, col, gs[i]);
    
    /*
    for (int i = 0; i<vs_4.length(); i++) col = plot(uv, col, vs_4[i], 
                                        vs_3[i]); 
    */    
	/*
